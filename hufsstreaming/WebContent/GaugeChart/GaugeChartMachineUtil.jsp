...
<body>
	...
	<%
		...
		try {
			...
			String query_str = "select caseid, activity, machine, time from bpi."+Database+"";
			...
			String q = "Select distinct machine from "+Database+""; // q 에 엑티비티들을 따온다
			...
			String q2 = "Select TIMESTAMPDIFF(SECOND,MIN(STR_TO_DATE(time,'%d/%m/%Y %h:%i:%s')),MAX(STR_TO_DATE(time,'%d/%m/%Y %h:%i:%s'))) AS TotalactTime FROM "+Database+" ";
			...
			for (int j = 0; j < check.length; j++) {
				...
				query_str1 += "FROM    "+Database+" as a ";

				query_str1 += "LEFT JOIN "+Database+" as b ";
				...
				int countNum = 0;
				String q5 = "Select time from "+Database+" where machine = '"
						+ check[j] + "'";
